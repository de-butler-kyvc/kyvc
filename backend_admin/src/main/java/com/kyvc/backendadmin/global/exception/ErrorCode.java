...
public enum ErrorCode {
    ...
    INVALID_USER_STATUS(HttpStatus.BAD_REQUEST, "INVALID_USER_STATUS", "유효하지 않은 사용자 상태입니다."),
    INVALID_USER_TYPE(HttpStatus.BAD_REQUEST, "INVALID_USER_TYPE", "유효하지 않은 사용자 유형입니다."),
    USER_ALREADY_WITHDRAWN(HttpStatus.CONFLICT, "USER_ALREADY_WITHDRAWN", "이미 탈퇴 처리된 사용자입니다."),
    USER_HAS_ACTIVE_KYC(HttpStatus.CONFLICT, "USER_HAS_ACTIVE_KYC", "진행 중인 KYC가 있어 삭제할 수 없습니다."),
    USER_HAS_VALID_CREDENTIAL(HttpStatus.CONFLICT, "USER_HAS_VALID_CREDENTIAL", "유효한 Credential이 있어 삭제할 수 없습니다."),
    ...
    KYC_DOCUMENT_NOT_FOUND(HttpStatus.NOT_FOUND, "KYC_DOCUMENT_NOT_FOUND", "KYC 문서를 찾을 수 없습니다."),
    DOCUMENT_DELETE_REQUEST_NOT_FOUND(HttpStatus.NOT_FOUND, "DOCUMENT_DELETE_REQUEST_NOT_FOUND", "문서 삭제 요청을 찾을 수 없습니다."),
    DOCUMENT_DELETE_REQUEST_ALREADY_PROCESSED(HttpStatus.CONFLICT, "DOCUMENT_DELETE_REQUEST_ALREADY_PROCESSED", "이미 처리된 문서 삭제 요청입니다."),
    INVALID_DOCUMENT_DELETE_REQUEST_STATUS(HttpStatus.CONFLICT, "INVALID_DOCUMENT_DELETE_REQUEST_STATUS", "처리할 수 없는 문서 삭제 요청 상태입니다."),
    ORIGINAL_FILE_DELETE_FAILED(HttpStatus.INTERNAL_SERVER_ERROR, "ORIGINAL_FILE_DELETE_FAILED", "원본 파일 삭제에 실패했습니다."),
    CREDENTIAL_NOT_FOUND(HttpStatus.NOT_FOUND, "CREDENTIAL_NOT_FOUND", "Credential 정보를 찾을 수 없습니다."),
    INVALID_CREDENTIAL_STATUS(HttpStatus.CONFLICT, "INVALID_CREDENTIAL_STATUS", "처리할 수 없는 Credential 상태입니다."),
    CREDENTIAL_ALREADY_REVOKED(HttpStatus.CONFLICT, "CREDENTIAL_ALREADY_REVOKED", "이미 폐기된 Credential입니다."),
    BACKEND_CALL_FAILED(HttpStatus.BAD_GATEWAY, "BACKEND_CALL_FAILED", "Backend API 호출에 실패했습니다."),
    INVALID_REPORT_DATE_RANGE(HttpStatus.BAD_REQUEST, "INVALID_REPORT_DATE_RANGE", "리포트 조회 기간이 올바르지 않습니다."),
    INVALID_REPORT_FORMAT(HttpStatus.BAD_REQUEST, "INVALID_REPORT_FORMAT", "지원하지 않는 리포트 내보내기 형식입니다."),
    REPORT_DATA_NOT_FOUND(HttpStatus.NOT_FOUND, "REPORT_DATA_NOT_FOUND", "리포트 데이터를 찾을 수 없습니다."),
    REPORT_EXPORT_FAILED(HttpStatus.INTERNAL_SERVER_ERROR, "REPORT_EXPORT_FAILED", "리포트 내보내기에 실패했습니다."),
    VC_REISSUE_REQUEST_FAILED(HttpStatus.BAD_GATEWAY, "VC_REISSUE_REQUEST_FAILED", "VC 재발급 요청에 실패했습니다."),
    VC_REVOKE_REQUEST_FAILED(HttpStatus.BAD_GATEWAY, "VC_REVOKE_REQUEST_FAILED", "VC 폐기 요청에 실패했습니다."),
    ...
    VERIFIER_NOT_FOUND(HttpStatus.NOT_FOUND, "VERIFIER_NOT_FOUND", "Verifier 정보를 찾을 수 없습니다."),
    VERIFIER_ALREADY_EXISTS(HttpStatus.CONFLICT, "VERIFIER_ALREADY_EXISTS", "이미 존재하는 Verifier입니다."),
    INVALID_VERIFIER_STATUS(HttpStatus.CONFLICT, "INVALID_VERIFIER_STATUS", "현재 상태에서는 Verifier 상태를 변경할 수 없습니다."),
    VERIFIER_API_KEY_NOT_FOUND(HttpStatus.NOT_FOUND, "VERIFIER_API_KEY_NOT_FOUND", "Verifier API Key를 찾을 수 없습니다."),
    INVALID_CALLBACK_URL(HttpStatus.BAD_REQUEST, "INVALID_CALLBACK_URL", "Callback URL 형식이 올바르지 않습니다."),
    NOTIFICATION_TEMPLATE_NOT_FOUND(HttpStatus.NOT_FOUND, "NOTIFICATION_TEMPLATE_NOT_FOUND", "알림 템플릿을 찾을 수 없습니다."),
    NOTIFICATION_TEMPLATE_ALREADY_EXISTS(HttpStatus.CONFLICT, "NOTIFICATION_TEMPLATE_ALREADY_EXISTS", "이미 존재하는 알림 템플릿입니다."),
    ...
    ADMIN_ROLE_DUPLICATED(HttpStatus.CONFLICT, "ADMIN_ROLE_DUPLICATED", "이미 존재하는 관리자 권한 코드입니다."),
    INVALID_ADMIN_ROLE_CODE(HttpStatus.BAD_REQUEST, "INVALID_ADMIN_ROLE_CODE", "유효하지 않은 관리자 권한 코드입니다."),
    INVALID_ADMIN_ROLE_STATUS(HttpStatus.BAD_REQUEST, "INVALID_ADMIN_ROLE_STATUS", "유효하지 않은 관리자 권한 상태입니다."),
    ADMIN_PERMISSION_DENIED(HttpStatus.FORBIDDEN, "ADMIN_PERMISSION_DENIED", "관리자 권한이 부족합니다."),
    ...
    KYC_SUPPLEMENT_NOT_FOUND(HttpStatus.NOT_FOUND, "KYC_SUPPLEMENT_NOT_FOUND", "KYC 보완요청을 찾을 수 없습니다."),
    KYC_SUPPLEMENT_NOT_SUBMITTED(HttpStatus.CONFLICT, "KYC_SUPPLEMENT_NOT_SUBMITTED", "제출 완료된 보완요청만 처리할 수 있습니다."),
    KYC_SUPPLEMENT_ALREADY_COMPLETED(HttpStatus.CONFLICT, "KYC_SUPPLEMENT_ALREADY_COMPLETED", "이미 처리 완료된 보완요청입니다."),
    KYC_SUPPLEMENT_EXPIRED(HttpStatus.CONFLICT, "KYC_SUPPLEMENT_EXPIRED", "만료된 보완요청은 처리할 수 없습니다."),
    KYC_SUPPLEMENT_DOCUMENT_NOT_FOUND(HttpStatus.NOT_FOUND, "KYC_SUPPLEMENT_DOCUMENT_NOT_FOUND", "보완 제출 문서를 찾을 수 없습니다."),
    INVALID_KYC_STATUS_TRANSITION(HttpStatus.CONFLICT, "INVALID_KYC_STATUS_TRANSITION", "허용되지 않는 KYC 상태 전이입니다."),
    ...
    DOCUMENT_REQUIREMENT_DUPLICATED(HttpStatus.CONFLICT, "DOCUMENT_REQUIREMENT_DUPLICATED", "이미 존재하는 필수서류 정책입니다."),
