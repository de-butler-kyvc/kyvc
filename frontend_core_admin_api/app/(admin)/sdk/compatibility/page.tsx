...
const compatibility = [
  { sdkVer: "2.3.1", coreVer: "v1.5", compat: "호환", note: "현재 적용" },
  { sdkVer: "2.2.0", coreVer: "v1.4", compat: "호환", note: "" },
  { sdkVer: "2.0.0", coreVer: "v1.2", compat: "미호환", note: "v1.3 미지원" },
];

export default function SdkCompatibilityPage() {
  return (
    <div>
      <PageHeader breadcrumb="SDK > 버전 호환성" title="SDK 버전 호환성 조회" />
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-100 bg-slate-50">
              {["SDK 버전", "코어 API 버전", "호환 여부", "비고"].map((h) => (
                <th key={h} className="text-left text-[11px] text-slate-400 font-medium px-4 py-2">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {compatibility.map((c) => (
              <tr key={c.sdkVer} className="border-b border-slate-50 hover:bg-slate-50 last:border-0">
                <td className="px-4 py-2.5 text-xs font-mono text-slate-700">{c.sdkVer}</td>
                <td className="px-4 py-2.5 text-xs text-slate-600">{c.coreVer}</td>
                <td className="px-4 py-2.5"><StatusBadge status={c.compat} /></td>
