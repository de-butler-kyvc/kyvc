...
import { ApiError, kyc as kycApi, type KycApplicationResponse } from "@/lib/api";
...
export default function CorporateKycHistoryPage() {
  ...
  const [rows, setRows] = useState<KycApplicationResponse[]>([]);
  ...
  useEffect(() => {
    ...
    kycApi
      ...
      .then((res) => {
        if (cancelled) return;
        setRows(Array.isArray(res) ? res : res?.kycId ? [res] : []);
      })
      .catch((err: unknown) => {
        if (cancelled) return;
  ...
  const filtered = useMemo(
    () =>
      rows.filter((row) => {
        ...
        if (tab === "supplement") return status === "SUPPLEMENT_REQUESTED";
          ...
          {loading ? (
            ...
            filtered.map((row) => (
              <tr key={row.kycId}>
                ...
                <td className="text-muted-foreground">{DONE.has(row.kycStatus ?? "") ? formatDate(row.updatedAt) : "-"}</td>
                <td>
                  <Badge variant={variant(row.kycStatus)}>{label(row.kycStatus)}</Badge>
                </td>
