...
- XLS-40d DID: 발급자 DID는 did:xrpl:1:{account} 형식을 사용하며, DID 문서 해시와 URI를 XRPL DIDSet 트랜잭션으로 등록한다.
- XLS-70d Credentials: 자격증명 상태는 XRPL CredentialCreate, CredentialAccept, CredentialDelete 흐름과 CredentialType으로 연결한다.
- Core는 SD-JWT 또는 vc+jwt 원문을 XRPL에 올리지 않고, XRPL은 발급자, 홀더, CredentialType 기준의 상태 원장으로 사용한다.
- Verifier는 자격증명 서명과 payload를 검증한 뒤 XRPL에서 CredentialType 상태를 조회한다.
...
- 오픈API JSON: {BASE_URL}/openapi.json
...
- APP_ENV=dev 기준으로 로컬 MySQL과 XRPL testnet을 사용한다.
- XRPL JSON-RPC는 testnet 엔드포인트를 설정해 사용한다.
