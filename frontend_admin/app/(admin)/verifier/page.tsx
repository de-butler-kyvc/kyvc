...
import { getVerifierList } from "@/lib/api/verifier";
...
export default function VerifierPage() {
  ...
  const fetchVerifierList = async () => {
    ...
    try {
      ...
      console.error("Failed to fetch verifier list:", error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-slate-200">
        <div className="flex items-center gap-2 p-4 border-b border-slate-100">
          ...
          <button className="ml-auto bg-blue-600 text-white px-4 py-1.5 rounded text-sm hover:bg-blue-700">+ 플랫폼 등록</button>
        </div>
            ...
            <tbody>
              {paginatedList.map((row) => (
                <tr key={row.id} className={`border-b border-slate-50 hover:bg-slate-50 transition-colors ${selectedIds.has(row.id) ? "bg-blue-50" : ""}`}>
                  ...
                  <td className="px-4 py-3">
                    <Link href={`/verifier/${row.id}`} className="text-blue-600 hover:underline text-xs">상세 →</Link>
                  </td>
                </tr>
              ))}
            </tbody>
